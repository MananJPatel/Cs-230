...
WordSet *wset_new () {
  ...
  WordSet *nodewithsize = (WordSet *)(malloc(sizeof(WordSet)));
  assert(nodewithsize != NULL);
  nodewithsize->size = 0;
  nodewithsize->head = NULL;
  return nodewithsize;  
}
...
static WordNode **search (WordSet *wset, char *word) {
  ...
  WordNode **point = &(wset->head);
  // put loop here
  while((*point) != NULL){
      if(strcmp((*point)->word,word) == 0){
        return point;
      }
        point = &((*point)->next);
  }
...
int wset_add (WordSet *wset, char *word) {
  ...
  WordNode **point = search(wset,word);
...
WordNode **point = search(wset,word);
  ...
  if((*point) != NULL){
    WordNode *OneMoreNode = (*point);
    *point = (*point)->next;
    OneMoreNode->next = wset->head;
    wset->head = OneMoreNode;
    return ++((wset->head)->freq);
  }
  else{
    (wset->size)++;
    WordNode *OneMoreNode = node_new(word);
    OneMoreNode->next = wset->head;
    wset->head = OneMoreNode;
    return ++(OneMoreNode->freq);
  } 
}
...
int wset_remove (WordSet *wset, char *word) {
  ...
  WordNode **pointer = search(wset,word);

  if(*pointer == NULL){
     return -1;
  }
   (wset->size)--;
   WordNode *OneMoreNode = *pointer;
   int freq1 = OneMoreNode ->freq;
